def preberi(ime_datoteke):
    ...
    for vrstica in datoteka:
        ...
        nov1 = nov.replace('|', "")
        nov2 = nov1.replace(",","")
        koordinate = nov2.split()
        i = 2
        while i < len(koordinate):
            slovar.setdefault((float(koordinate[0]),float(koordinate[1])), []).append((float(koordinate[i]),
                                                                                       float(koordinate[i + 1])))
            i += 2
    return slovar
...
def vrstica(i, ladja, marsovec):
    ...
    if isinstance(i, int):
        for k in range(len(ladja)):
            return f"{i:4}:{ladja[k]:11.4f} {ladja[k + 1]:10.4f} -> {marsovec[k]:10.4f} {marsovec[k + 1]:10.4f}"
    elif i is None:
        for k in range(len(marsovec)):
            return f"{marsovec[k]:41.4f} {marsovec[k + 1]:10.4f}"
...
def navodila(razpored, ime_datoteke):
    ...
    for kljuc in razpored:
        i += 1
        kljuc1 = 0
        for vrednosti in razpored.get(kljuc):
            if kljuc != kljuc1:
                v = f"{vrstica(i,kljuc,vrednosti)}\n"
                kljuc1 = kljuc
            else:
                v = v + f"{vrstica(None,kljuc,vrednosti)}\n"
        s = s + v
    datoteka.write(s)
