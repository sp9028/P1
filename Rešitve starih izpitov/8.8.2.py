...
def najpogostejse_polje(navodila):
    ...
    return max(polja, key=polja.get)
...
def navodila(zaporedje):
    ...
    while zaporedje:
        if zaporedje[0] == "D":
            if len(txt) > 0:
                seznam.append(int(txt))
                txt = ""
            seznam.append(zaporedje[0])
        elif zaporedje[0] == "L":
            if len(txt) > 0:
                seznam.append(int(txt))
                txt = ""
...
class Carovnik:
    ...
    def __init__(self, niz, cena):
        self.niz = niz
        self.cena = cena
        self.skupni_zasluzek = 0

    def caraj(self, s):
        nov = []
        stara,nova = self.niz.split("->")
        index = 0
        for element in s:
            if element == stara.strip():
                nov.append(nova.strip())
                self.skupni_zasluzek += self.cena
            else:
                nov.append(element)
        return nov

    def zasluzek(self):
        return self.skupni_zasluzek
