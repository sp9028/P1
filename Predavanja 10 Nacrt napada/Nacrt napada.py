def preberi(ime_datoteke):
    ...
    for vrstica in open(ime_datoteke):
        nase_ladje, marsovci = vrstica.split(':')
        x,y = nase_ladje.split(",")
        nase_ladje = (float(x),float(y))
        koordinate = []
        for marsovci in marsovci.split('|'):
            x,y = marsovci.split(",")
            koordinate.append((float(x), float(y)))
        slovar[nase_ladje] = koordinate
...
def vrstica(i, ladja, marsovec):
    if i is not None:
        return f"{i:4}:{ladja[0]:11.4f} {ladja[1]:10.4f} ->{marsovec[0]:11.4f} {marsovec[1]:10.4f}"
    else:
...
def navodila(razpored, ime_datoteke):
    ...
    for i,(ladje,marsovec) in enumerate(razpored.items(), start=1):
        datoteka.write(vrstica(i,ladje,marsovec[0]) + "\n")
        for marsovec in marsovec[1:]:
            datoteka.write(vrstica(None,ladje,marsovec) + "\n")
...
class Test(unittest.TestCase):
    ...
    def test_preberi(self):
        ...
        try:
            ...
            os.rename("../Vaje za izpit 2/jozica-kozjanc.txt", fname)
            self.assertEqual(
                {(4.25, 8.5): [(5.0, 5.0), (8.125, -7.5), (3.5, -6.75)],
                 (6.0, 1.25): [(15.5, 5.5), (1.5, -1.25), (2.5, -9.5625), (-3.0, -3.0)],
                 (8.0, 1.0):  [(-2.0, 5.0)]},
            ...
            os.rename(fname, "../Vaje za izpit 2/jozica-kozjanc.txt")
